...
- **페이지 목록**
  ...
  - `/login` — 로그인 페이지 (**소셜 전용** — Google·네이버·카카오 와이드 버튼 + Twitch·치지직·SOOP 원형, 팝업 OAuth 흐름)
...
- **폼 검증**: `react-hook-form` + `zod` — 로그인 전용 스키마는 제거, 회원가입·프로필 등 필요 경로만 유지 (`src/lib/schemas/auth.ts` 등)
...
### 세션 메모 (2026-03-26 — 티어 라벨 커스텀·로그인 UX·파비콘·브랜드)

- **티어 라벨 커스텀**: `tier-label-surface.ts` — 라벨 영역 표면(라이트/다크) 토큰·`tier-store` 상태 확장, `tier-settings-modal`에서 라벨 스타일 편집, `tier-row`·`tier-board-readonly`에 반영. **`rewriteTiersUploadUrls`** (`pickty-image-url.ts`) — 티어 행 **`backgroundUrl`**(행 배경 이미지)도 업로드 URL 재작성 대상에 포함.
- **로그인 UX**: 이메일·비밀번호·구분선 제거 — **소셜 전용** 카드 (`login/page.tsx`). 방송 계열 원형 버튼(Twitch·치지직·SOOP) 순서 유지, 준비 중 안내는 **`sonner`** (`SonnerToaster` in `layout.tsx`). 치지직/SOOP 아이콘은 **`next/image`** — **`/brand/2.png`**, **`/brand/1.png`**. GNB 티어표 네비 active 스타일·로그인/테마 토글 **`h-9`** 정렬.
- **브랜드·파비콘**: Material **`local_parking`** 마젠타 P(약 `#EA33F7`) 소스로 **`public/brand/pickty-mark-transparent-{120,128,140}.png`** 투명 PNG 생성. **`metadata.icons`** 및 **`src/app/icon.png`** → **`/brand/pickty-mark-transparent-128.png`**. OAuth 콘솔 등에 PNG가 필요하면 동일 폴더 자산 사용.
