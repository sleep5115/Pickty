...
- `docker-compose.yml` — PostgreSQL 17 + Valkey 9 **선택(로컬 PC 전용)**. 호스트 **5442·6380** → 컨테이너 5432·6379. **`POSTGRES_PASSWORD`** 는 루트 **`.env`**. 공용 개발은 Lightsail 등 — 프로필 **`dev`** + `application-secrets.yaml`
...
- **개발 단계 운용 (합의)**  
  - **PostgreSQL / Valkey**: **Lightsail Ubuntu + Docker** 등 **단일 Postgres**에 **`pickty_dev` / `pickty_prod`** 로 분리. 로컬 개발 PC는 기본 **`dev`**(공용 DB면 `DB_HOST`/`VALKEY_HOST`) 또는 **`local`**(로컬 compose만). OAuth·JWT는 **`application-secrets.yaml`**; **`local`** 전용 연결은 **`application-local.yaml`**. (USB 동기화 — PROGRESS에 실값 금지.)  
...
| 방화벽 | **TCP 22** (SSH), **80**, **443** 오픈. 로컬 PC에서 DB·Valkey 직접 붙이면 **5432·6379** 추가 — **가능하면 고정 IP만** 허용 |
...
1. Lightsail 네트워킹에서 **TCP 5432** 허용(가능하면 **본인 고정 IP**만).
...
- **로컬 dev vs 운영 DB 혼선(해결 체크)**: IntelliJ **Active profiles: `dev`** + 프론트 `npm run dev`로 검증 시 로컬 API는 **`pickty_dev`**에 저장되고, **`pickty.app`**(배포 API)는 **`pickty_prod`**에 저장되는 것이 정상임을 확인. **`prod` 프로필**로 로컬을 띄우거나 API 베이스 URL이 섞이면 DB가 달라 보일 수 있음 — 재현 없이 정상 동작 확인되어 **이슈 해결로 정리**.
...
- **pickty-config**: `application-secrets.yaml` · **`application-local.yaml`**(Lightsail `pickty_dev` + Valkey, secrets 변수) · `.env` · **PEM** · `frontend.env.local`(HTTPS API/사이트 URL). (Gradle JDK 샘플은 Pickty **`backend/gradle.properties.example`**)
...
#    (선택) copy backend\gradle.properties.example backend\gradle.properties
...
### application-secrets.yaml / application-local / Gradle JDK

**선택 로컬 Docker**: 호스트 **Postgres 5442**, **Valkey 6380** — `docker-compose.yml` 전용. 일상 개발은 Lightsail DB/Valkey(`dev`/`local` + secrets).

**JDK 경로**: **`JAVA_HOME`** 또는 IntelliJ SDK 권장. 미설정 시 `backend/gradle.properties.example` → `gradle.properties` 복사 후 `REPLACE_ME` 참고.
...
| `gradle.properties` | `backend/` | (선택) 레포 `gradle.properties.example` → `gradle.properties` 로 복사 |
| `.env` | Pickty 루트 | Docker 비번 |
| `Lightsail…pem` | Pickty 루트 | SSH용 |

#### pickty-config 정본 유지

- pickty-config 의 secrets·`application-local`·`.env`·`.pem`·`frontend.env.local` 이 Pickty에 복사본과 **일치하는지** 확인 후, 수정은 **pickty-config만** 커밋·푸시.
...
- **마크다운 정본**: `frontend/content/legal/TERMS_OF_SERVICE_KO.md`, `PRIVACY_POLICY_KO.md` 단일 — 레포 루트 동명 파일·`sync-legal-docs` 스크립트 제거, `read-legal-doc.ts`·`package.json`에서 해당 경로만 사용.
- **후속 (2026-03-31)**: 약관 제2조 **비회원** — 댓글 등에 **IP 일부 화면 공개 표기**; 제7조 — **무단 크롤링·스크래핑·AI 학습 무단 이용** 금지. 개인정보처리방침 — 서두 최소 수집 문구, 비회원 표 **처리·이용 목적**·**IP 일부 마스킹 노출**. 상세·커밋 메시지는 루트 **`PROGRESS.md`** 「법무 웹 노출」절.
...
- **`ResultVoteButtons`**: 카드(`sm`)는 추천·비추천 **회색 없이** 항상 빨강·파랑; 상세(`lg`)도 톤 고정, **내 선택만 테두리 글로우**(`box-shadow`). 카드는 테두리 없음·상세는 라운드 박스 유지.
- **`TemplateLikeButton`**: 카드 `appearance="plain"`, 티어 헤더 등 `boxed`.
- **마이그레이션**: `docs/migrations/2026-03-30-reactions-member-ip-hash-hybrid.sql` — 회원 행에도 `guest_ip_hash` 허용·부분 유니크로 NAT 내 회원 간 충돌 방지.

## 인기 티어표 문구 **(2026-03-30)**

- `popular-tier-results`: 안내 문구 **「이 템플릿으로 만든 티어표 중…」** 으로 정리(결과 vs 티어표 표현).
