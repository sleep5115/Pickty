...
### 2.1. [이상형 월드컵] 자동화 대진 동기화 흐름 (In-Game Auto Sync)
...
sequenceDiagram
    ...
    Note over S: 일반 월드컵 인게임 플레이 시작
    V->>B: 참여 URL 접속 -> 역할 선택 (방장 투표 참여 vs 개별 플레이)
    
    Note over S: [16강 1경기] 피카츄 vs 라이츄 카드 자동 로드
    S->>B: [백그라운드 자동 송출] 현재 매치 정보 업데이트 (leftId: 피카츄, rightId: 라이츄)
    V->>B: [폴링 감지] 현재 매치 감지 & UI 전환 (버전 ETag 기반)
    V->>B: 실시간 투표 제출 (피카츄 선택)
    
    Note over S: [방장 플레이 화면] 카드 하단에 실시간 득표율 게이지 자동 표출 (SSE 1초 스로틀 수신)
    S->>S: 카드 클릭하여 피카츄 승리 선택 (다음 2경기 카드로 진행)
    S->>B: [백그라운드 자동 송출] 2경기 매치 정보 업데이트 (꼬부기 vs 파이리)
    V->>B: [폴링 감지] 새 매치 정보 수신 및 투표 초기화
```

1. **세션 활성화**: 스트리머가 월드컵 상세에서 스트리머 모드를 시작하면 세션 ID와 `hostToken`이 발급됩니다.
2. **인게임 자동 동기화 (Auto Sync)**:
   * 방장은 수동으로 아이템을 골라 대진을 임의 설정하지 않습니다. **기존의 일반 월드컵 게임 진행 스택(`useWorldCupStore`)을 그대로 사용**합니다.
   * 방장 화면에서 매 대진(예: 피카츄 vs 라이츄) 카드가 렌더링될 때마다, 프론트엔드가 백그라운드에서 현재 매치 업데이트 API(`PUT /sessions/{id}/match`)를 자동으로 호출합니다.
   * 스트리머가 카드를 클릭하여 승자를 정하면 다음 매치로 스토어가 전진하고, 프론트엔드는 다음 매치 정보를 즉시 서버로 송출합니다.
3. **방장 화면 실시간 게이지 오버레이**:
   * 방장 플레이룸 카드 하단에 시청자 투표율이 실시간으로 표출되며, 방장은 이를 보고 클릭 결정을 조율할 수 있습니다.
...
### 7.2. 방장 인게임 플레이 및 자동 동기화 UI (Streamer Host Mode)
* 스트리머 모드가 활성화된 방장은 별개의 매치 지정용 대시보드로 이동하지 않고, **기존의 일반 월드컵 인게임 플레이 페이지(`/worldcup/templates/[id]`)에서 그대로 월드컵을 플레이**합니다.
* **자동 매칭 송출:** 방장 화면에서 다음 대결 카드(예: 피카츄 vs 라이츄)가 스토어에 얹히는 시점(인덱스 변경 감지 시)에 프론트엔드가 백그라운드에서 `PUT /sessions/{sessionId}/match`를 **자동 호출**해 현재 대진 정보를 Valkey에 갱신합니다.
* **실시간 게이지 연동:** 방장 화면의 두 대결 대상 카드 바로 하단 영역에, 시청자가 실시간 투표한 비율(SSE 수신 정보)이 게이지 바와 득표수로 실시간 합성되어 보여집니다.
* **플레이 완료 종료:** 방장이 결승전까지 플레이하여 최종 우승자(Champion)를 가리고 결과 화면으로 이동하면, 자동으로 `POST /finish` API가 트리거되어 통계가 DB에 이관되고 세션이 종료됩니다.
