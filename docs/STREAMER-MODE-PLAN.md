...
* **~~월드컵 보르다 투표제 (Borda Count)와 데이터 이원화~~ — 전면 폐기(2026-06-08):**
  * **폐기 사유:** (b) "시청자 개별 완주 후 제출"은 ① 시청자별 랜덤 대진이라 트리 평균이 구조적으로 불가능(파생 랭킹만 가능), ② 대량 아이템·고강수(256강 등)에서 완주 동선이 비현실적, ③ 방장이 가지치기한 아이템의 이후 전적은 데이터가 애초에 없어 복원 불가. 대안으로 검토한 **"방장 종료 후 시청자 다수결 자동 토너먼트"**도 시작 강수↔유저 우승자 탈락 모순·256강 13분 방송 공백 등으로 폐기.
  * **결론:** 월드컵은 **(a) 실시간 대진 투표만** 남긴다. 이는 방장 실시간 소통용 보조 지표이며 별도 종합 랭킹 집계는 하지 않는다. (티어표 개별 제출·평균 집계는 §2.2대로 유지 — 폐기 대상 아님.)
...
> **[변경 2026-06-08]** 월드컵 개별 플레이 폐기로 **월드컵 시청자에겐 선택지가 하나뿐**이라 진입 선택 화면(`EntrySelector`)을 제거했다. 월드컵 시청자는 URL 진입 즉시 "방장과 함께 투표" 화면으로 직행한다. 티어표 시청자는 원래부터 선택 화면 없이 `TierViewerClient`(개별 완성→제출)로 직행하므로 무관.

(원래 기획) 시청자가 공유받은 스트리머 참여 URL로 최초 진입 시, 본인의 디바이스 상황에 맞는 방식을 명확히 인지하고 선택할 수 있도록 단순한 선택지를 제공합니다.
* **[선택 1] 방장과 함께 투표하기 (추천: 모바일/가벼운 참여)**
  * 스트리머가 지목한 아이템이나 월드컵 현재 대진에 맞춰 실시간으로 표만 던지는 가벼운 모드입니다.
* **~~[선택 2] 나만의 티어표/월드컵 완성하기~~** — **월드컵분 폐기(2026-06-08)**, 티어표 개별 완성·제출만 유지.
...
* **~~완성 리포트: 월드컵 Borda Count 시청자 종합 랭킹~~ — 폐기(2026-06-08)**: 시청자 월드컵 개별 완주가 폐기되어 종합 랭킹 데이터 소스가 없음. 월드컵은 실시간 대진 투표만 남김. (티어표 비교 리포트는 §2.2·'내 스트리밍'으로 별도 유지.)
