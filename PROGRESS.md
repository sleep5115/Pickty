...
- **구현: Auth 스키마·온보딩**: `users` — `user_name`, `account_status`, `gender`, `birth_year`, **`onboarding_completed_at`**, **`display_avatar_url`**(업로드 공개 아바타), **`profile_image_url`**(소셜 제공 이미지 링크); `social_accounts.provider_attributes`(jsonb). 신규 소셜 가입 닉네임 **`User_{id}`**. 공개 **`GET /me`** 의 `profileImageUrl` JSON 필드는 **`display_avatar_url`** 만. **`GET /me/sensitive`** · **JWT email 제거** 등은 기존과 동일. **`/signup/profile`**: 카피 정리(「온보딩」 비노출), 프로필 사진은 **접이식 + 파일 업로드·DnD** → `POST /api/v1/images` 후 **`PATCH ... displayAvatarUrl`**.
