...
- **이미지 저장(현재)**: `POST /api/v1/images` → **AWS SDK S3 호환 클라이언트**로 **Cloudflare R2** `PutObject`. DB `items` JSON에는 **`https://img.pickty.app/…`**(또는 설정한 `public-url`) 형태. 프론트 기본 API는 **`https://api.pickty.app`**, 사이트 **`https://pickty.app`** — Mixed Content 방지 전제.
...
- **CORS**: API는 `SecurityConfig`의 `/**` CORS(허용 origins·`http://localhost:*`). 이미지는 **R2 공개 도메인**에서 서빙 — **R2 버킷 CORS**를 프론트·캡처(html-to-image)에 맞게 설정할 것.
- **`<img crossOrigin>`**: 레거시 `/uploads/`·**`img.pickty.app`**·**`*.r2.dev`** Pickty 자산 URL에 적용 — 외부 이미지 엑박 방지.
...
- [x] **Frontend**
  ...
  - `/tier/result/[id]` — `TierBoardReadonly` + **PNG 다운로드** (캡처 ref는 티어 행만·미분류 풀 제외 — 메이커 `ExportModal`과 동일) · R2 이미지는 **버킷 CORS**·`next.config` `images.remotePatterns`(`img.pickty.app`, `*.r2.dev`, `api.pickty.app`) 확인
