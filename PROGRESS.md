...
- **P3 게시판 프론트 1차 (작성·목록·라우팅)**: `**/board**`·`**/board/write**`·`**/board/[id]`(stub) 추가, GNB **커뮤니티** 탭(PC/모바일) 연결. 목록은 **컴팩트 행형 UI** + 제목 옆 이미지 아이콘 hover 시 **썸네일 미리보기**로 변경.
...
- **P3 게시판(백엔드 API+DB·프론트 연동) 완료 및 배포 반영**: `board_posts` 마이그레이션·엔티티·리포지토리·서비스·컨트롤러(작성/목록/상세) 추가, 비회원 정책(닉네임/비번·IP 해시) 및 HTML sanitize 적용. 프론트는 `/board` 목록·`/board/write` 작성·`/board/[id]` 상세를 실 API와 연결하고, 작성 완료 후 토스트 + 목록 복귀 UX로 정리. 에디터는 최종적으로 **이미지 업로드·유튜브 링크·일반 링크** 3기능 중심으로 정리. 반영 커밋(`dev`): `0d58390`, 운영 반영용 `main` 머지 커밋: `8205a36`.

---

## 진행 메모 **(2026-04-07)**

- **게시판 댓글 연동 (BOARD_POST) 1차 완료**: `comments.target_type`은 기존부터 `varchar(32)`로 `BOARD_POST` 문자열 저장은 가능했으나, 서비스에서 `NOT_IMPLEMENTED`로 막고 있었다. `CommunityCommentService`에 `BOARD_POST` 허용·대상 검증(`board_posts` ACTIVE)·댓글 수 증감 로직을 추가하고, `board_posts.comment_count` 역정규화 컬럼 마이그레이션(`docs/migrations/2026-04-07-board-post-comment-count.sql`)을 반영.
- **게시글 상세 응답에 댓글 포함**: `GET /api/v1/board/posts/{id}`에서 `CommunityCommentService`를 호출해 댓글 첫 페이지(기본 30개)를 `comments`로 함께 반환하도록 DTO·서비스를 확장.
- **프론트 상세 하단 댓글 UI 연결**: `/board/[id]` 하단에 `CommentSection`을 `targetType="BOARD_POST"`·`targetId={post.id}`로 연결. 상세 API의 `comments`를 초기값으로 주입해 첫 렌더에서 불필요한 추가 목록 호출을 피함.
...
| **P3 커뮤니티 게시판** (TipTap·리치 에디터)           | 🟨(진행중) | 1차 기반(DB·작성/목록/상세 API·프론트 연동) + **게시판 댓글(`BOARD_POST`) 연동** 완료(2026-04-07). **잔여**: 게시글 **수정**, **삭제(소프트 삭제)**, 게시글 **추천/비추천**, 작성/상세 UX·권한/예외 처리 등 **디테일 보완**. 에디터 기능 범위는 **이미지 업로드·유튜브 링크·일반 링크** 3축 유지. |
...
| **Ideal Type 이상형 월드컵**                      | ⬜     | **P3 게시판 마무리 후** 착수. GNB·라우트 **UI 비노출** 유지. 스트리머·집계 확장과 별개 순서(「장기 아이디어」).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- **당장 로드맵에 두지 않음**: 집계 티어표·댓글 멘션 UI·글로벌 알림·스트리머·월드컵(월드컵은 **게시판 다음**으로 「다음 작업」에만 명시) — 상세 **「장기 아이디어」** 절.

### Phase 3 — 구현

- 티어 템플릿·이미지·`tiers/results`·프론트(R2) — **진행됨**
- **P3 게시판** — **당면**
- **이상형 월드컵** — **게시판 후**(세부는 장기 절·기획 보관본)
...
6. **Ideal Type 이상형 월드컵** — **P3 게시판 마무리 후** 착수. UI 비노출 유지 → 구현 시 노출·라우트 결정.  
...
- 스트리머·대규모 트래픽 방어 기획은 **「장기 아이디어」** 절. **제품 순서**는 「다음 작업」— **게시판 → 이상형 월드컵**.
