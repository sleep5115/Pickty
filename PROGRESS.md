...
- **커뮤니티 게시글 상세 UI 일관성**: `**/community/[id]/page.tsx**` — 본문(제목·작성자/날짜/조회수·HTML)을 **`CommentSection` 과 동일 카드 스타일** 공통 상수 `**COMMUNITY_CARD_SECTION_CLASS**`(`**/lib/community-ui.ts**`)로 감쌈; `**comment-section.tsx**` 루트도 동일 상수 사용해 시각 규격 단일화. 상단 **「← 목록으로」** 링크 **제거**. **게시글 카드**와 **댓글 카드** 사이 **`gap-6`** + 우측 **`flex justify-end`** 로 **수정**(연두 계열)·**삭제**(빨강 계열)·**목록**(보라 계열) 배치 — 수정·삭제는 **로그인 작성자 본인**에게만 표시, API 미구현 시 **`sonner`** 토스트로 준비 중 안내.
