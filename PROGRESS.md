...
| **AI 자동 템플릿 생성 (AI 딸깍)**             | 🟨(진행중) | **(2026-04-24 후속)** `**POST /api/v1/admin/ai/auto-generate**` + `**AiGenerationPanel**` — 티어·월드컵 에디터 **ADMIN** 전용. PHOTO/GIF/YOUTUBE·개수(1~50)·응답 **`candidates[]`**(`url`·`title?`)·유튜브 제목 매핑. 월드컵 편집기: 깡통 행 정리·저장 시 미디어 검증·**미리보기 모달**·채택 시 이름 동기화. 구 `**/admin/templates/auto-generate**` 경로는 폐기. Phase 3(Vision) 등은 여전히 미도입. |
