...
- GitHub 저장소 **Settings → Secrets and variables → Actions** 에 다음을 등록합니다.
  ...
  - **`APPLICATION_SECRETS_YAML`** — `pickty-config` 의 **`application-secrets.yaml` 파일 전체**를 그대로 붙여 넣기(멀티라인). 운영 컨테이너는 `SPRING_CONFIG_IMPORT` 로 이 파일을 읽으며, **Kakao/Google OAuth·JWT·R2·`FRONTEND_URL`** 등이 없으면 로그인이 실패합니다. `docker-compose.api.yml` 의 `DB_HOST` / `VALKEY_*` 는 여전히 Compose 환경변수로 덮어씁니다.  
    **비워 두면:** 서버에 이미 `~/Pickty/deploy/lightsail/application-secrets.yaml` 이 있으면 그대로 두고 배포만 진행한다(경고 로그). 없으면 워크플로가 실패한다. CI에서 시크릿을 단일 정본으로 쓰려면 반드시 채운다.
...
- **동작:** SSH 로 서버에 접속 → TLS·**`application-secrets.yaml`** 갱신(Secrets 기준) → `git pull origin main` → `docker compose ... down --remove-orphans` → `up -d --build`
- **`APPLICATION_SECRETS_YAML` Secret 이 비어 있으면:** 서버에 해당 경로 파일이 **이미 있으면** 유지 후 배포 계속, **없으면** 실패. 운영 정본을 Actions 로 맞추려면 Secret 을 채운다.
