 ...
 * - 컬럼 URL: [CloudflareR2Properties.publicUrl] 접두 + 키 형식
...
class ImageReferencedKeysReader(
    ...
    /** Postgres POSIX 정규식 — [R2ImageStorageService] 가 저장하는 확장자와 맞춤 */
    ...
    private fun keysFromUrlOrPlain(raw: String?): Set<String> {
        ...
        if (t.startsWith(publicBase, ignoreCase = true)) {
            ...
            if (R2ImageStorageService.STORED_OBJECT_KEY_REGEX.matches(path)) {
                out.add(path.lowercase())
            }
        }
        R2ImageStorageService.STORED_OBJECT_KEY_FRAGMENT_REGEX.findAll(t).forEach { m ->
            val candidate = m.value.lowercase()
            if (R2ImageStorageService.STORED_OBJECT_KEY_REGEX.matches(candidate)) {
                out.add(candidate)
            }
        }
        return out
